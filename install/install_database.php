...
if (!empty($form_data['brand']) && !empty($form_data['description']) && !empty($form_data['keywords']) && !empty($form_data['domain']) && !empty($form_data['admin_name']) && !empty($form_data['admin_email']) && !empty($form_data['admin_password'])) 
{
	...
	$query = "
		INSERT INTO `access_levels` (`id`, `resource`, `description`, `mask_a`, `mask_o`, `mask_u`, `mask_g`) VALUES
			...
			(13, 'get_users_list', 'Lista użytkowników', 1, 1, 0, 0),
			(14, 'get_user', 'Informacja o użytkowniku', 1, 1, 1, 0),
			(15, 'add_user', 'Dopisywanie użytkowników', 1, 0, 0, 0),
			(16, 'update_user', 'Modyfikacja użytkowników', 1, 0, 0, 0),
			(17, 'delete_user', 'Usuwanie użytkowników', 1, 1, 0, 0),
			(18, 'change_password', 'Zmiana hasła użytkownika', 1, 1, 1, 0),
			(19, 'logout_user', 'Zdalne wylogowanie użytkownika', 1, 1, 0, 0),
			(20, 'get_acl_list', 'Lista praw dostępu', 1, 1, 0, 0),
			(21, 'get_acl', 'Informacja o prawach dostępu', 1, 0, 0, 0),
			(22, 'add_acl', 'Dopisywanie praw dostępu', 1, 0, 0, 0),
			(23, 'update_acl', 'Modyfikacja praw dostępu', 1, 0, 0, 0),
			(24, 'delete_acl', 'Usuwanie praw dostępu', 1, 0, 0, 0),
			(25, 'get_user_rights', 'Informacja o prawach dostępu użytkownika', 1, 0, 0, 0),
			(26, 'update_user_rights', 'Zapis praw dostępu użytkownika', 1, 0, 0, 0),
			(27, 'get_messages_list', 'Lista wiadomości', 1, 0, 0, 0),
			(28, 'get_message', 'Informacja o wiadomości', 1, 1, 0, 0),
			(29, 'update_message', 'Modyfikacja wiadomości', 1, 1, 0, 0),
			(30, 'delete_message', 'Usuwanie wiadomości', 1, 0, 0, 0),
			(31, 'get_images_list', 'Lista obrazków galerii', 1, 1, 0, 0),
			(32, 'get_image', 'Pobieranie obrazków z galerii', 1, 1, 1, 1),
			(33, 'get_image_details', 'Pobieranie informacji o obrazku w galerii', 1, 1, 1, 1),
			(34, 'add_image', 'Dopisywanie obrazków do galerii', 1, 1, 0, 0),
			(35, 'update_image', 'Zmiana obrazka w galerii', 1, 1, 0, 0),
			(36, 'delete_image', 'Usuwanie obrazków z galerii', 1, 1, 0, 0),
			(37, 'get_categories_list', 'Lista kategorii', 1, 1, 0, 0),
			(38, 'get_category', 'Odczytanie szczegółów kategorii', 1, 1, 0, 0),
			(39, 'add_category', 'Dopisywanie kategorii', 1, 1, 0, 0),
			(40, 'update_category', 'Modyfikacja kategorii', 1, 1, 0, 0),
			(41, 'delete_category', 'Usuwanie kategorii', 1, 0, 0, 0),
			(42, 'move_category', 'Zmiana kolejności kategorii', 1, 0, 0, 0),
			(43, 'get_pages_list', 'Lista podstron serwisu', 1, 1, 0, 0),
			(44, 'get_page', 'Pobieranie danych o stronie', 1, 1, 0, 0),
			(45, 'add_page', 'Dopisywanie stron', 1, 1, 0, 0),
			(46, 'update_page', 'Modyfikacja stron', 1, 1, 0, 0),
			(47, 'delete_page', 'Usuwanie stron', 1, 0, 0, 0),
			(48, 'get_archives_list', 'Pobieranie listy archiwów strony', 1, 1, 0, 0),
			(49, 'get_archive', 'Pobieranie archiwalnej wersji strony', 1, 1, 0, 0),
			(50, 'archive_page', 'Archiwizowanie strony', 1, 1, 0, 0),
	...
	$query = "
		INSERT INTO `settings` (`id`, `key_name`, `key_value`, `meaning`, `modified`) VALUES
			...
			(10, 'visitors_excluded', '''192.168.1.0'', ''192.168.1.1''', 'Adresy IP wykluczone z wyświetlania w raporcie odwiedzin', NOW()),
			(11, 'visitors_period', '-180 days', 'Liczba ostatnich dni wczytywanych do raportu odwiedzin', NOW()),
			(12, 'visitors_referer', 'http', 'Filtr HTTP Referer przeglądarki użytkownika w raporcie odwiedzin', NOW()),
			...
			(14, 'carousel_interval', '5000', 'interwał czasowy wyświetlania obrazków w karuzeli slajdów', NOW()),
			(15, 'carousel_index_enabled', 'true', 'czy ma być włączona karuzela slajdów na stronie głównej', NOW()),
			(16, 'carousel_page_enabled', 'true', 'czy ma być włączona karuzela slajdów na podstronie', NOW()),
			(17, 'carousel_category_enabled', 'true', 'czy ma być włączona karuzela slajdów na kategorii', NOW()),
			(18, 'carousel_contact_enabled', 'true', 'czy ma być włączona karuzela slajdów na stronie kontaktowej', NOW()),
